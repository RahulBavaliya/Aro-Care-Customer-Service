...
export function useFilterManagement(): UseFilterManagementResult {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data: pendingData, error: pendingError } = await supabase
        .from('customers')
        .select('id, name, phone, next_service, filter_type')
      ...
      const filterStats: FilterStat[] = [
        {
          label: 'Due Today',
          value: pendingFilters.filter((p) => p.daysOverdue === 0).length,
          icon: AlertTriangle,
          color: 'bg-red-500',
        },
        {
  ...
  const addFilterChange = async (data: {
    customerId: string;
    filterType: string;
    changeDate: string;
    nextDue: string;
    ...
    try {
      const { data: insertData, error: insertError } = await supabase
        ...
        .select(); // so we can read the inserted row back
      ...
      if (insertError) {
        ...
        const { error: updateError } = await supabase
          .from('customers')
          .update({
            last_service: inserted.change_date,
            next_service: inserted.next_due,
          })
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to add filter change'
      );
    }
  };
